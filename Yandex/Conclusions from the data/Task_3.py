...
print("interval for aspirin infarct [%.4f, %.4f]" % (np.round(conf_interval_aspirin[0], 4),
                                                     np.round(conf_interval_aspirin[1], 4)))
print("interval for placebo infarct [%.4f, %.4f]" % (np.round(conf_interval_placebo[0], 4),
                                                     np.round(conf_interval_placebo[1], 4)))
...
print("interval for aspirin and placebo probability diff: [%.4f, %.4f]" % (
...
# %%
np.random.seed(0)
all_aspirin_odds = np.array(map(calculate_odds_from_samples,  get_bootstrap_samples(aspirin_vector, 1000)))
all_placebo_odds = np.array(map(calculate_odds_from_samples, get_bootstrap_samples(placebo_vector, 1000)))
...
# %%
print(np.round(stat_intervals(all_placebo_odds / all_aspirin_odds, 0.05), 4))
